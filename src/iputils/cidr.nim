...
proc rangeToCidr8[T: Cidr4|Cidr6](a, b: uint8, x: int): seq[T] =
  ...
  while a < b:
    let
      clzb = 15 - countLeadingZeroBits(b - a)
      ctza = countTrailingZeroBits(a or 256) # avoid undefined behavior when `a` is zero
      i = min(clzb, ctza)

    var cidr: T

    cidr.ip[x] = uint8(a)
    cidr.prefix = 8'u8 - uint8(i)
